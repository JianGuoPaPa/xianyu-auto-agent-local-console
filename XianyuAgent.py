...
class BaseAgent:
    ...
    def _is_openai_gpt5_model(self) -> bool:
        base_url = os.getenv("MODEL_BASE_URL", "")
        return "api.openai.com" in base_url and self._model_name().startswith("gpt-5")

    def _supports_provider_search(self) -> bool:
        search_enabled = os.getenv("ENABLE_MODEL_SEARCH", "").strip().lower()
        if search_enabled in {"1", "true", "yes", "on"}:
            return True
        if search_enabled in {"0", "false", "no", "off"}:
            return False

        base_url = os.getenv("MODEL_BASE_URL", "").lower()
        if any(host in base_url for host in ("localhost", "127.0.0.1", "0.0.0.0", "192.168.")):
            return False
        return "dashscope.aliyuncs.com" in base_url or "bailian.aliyuncs.com" in base_url

    def _openai_completion_token_limit(self, fallback: int) -> int:
        try:
            return int(os.getenv("OPENAI_MAX_COMPLETION_TOKENS", max(fallback, 1200)))
        except ValueError:
            return max(fallback, 1200)

    def _chat_completion(self, messages: List[Dict], temperature: float = 0.4, max_tokens: int = 500, top_p: float = 0.8, extra_body=None):
        kwargs = {
            "model": self._model_name(),
            "messages": messages,
        }

        if self._is_openai_gpt5_model():
            kwargs["max_completion_tokens"] = self._openai_completion_token_limit(max_tokens)
        else:
            kwargs.update({
            ...
            if extra_body and self._supports_provider_search():
...
class PriceAgent(BaseAgent):
    ...
    def generate(self, user_msg: str, item_desc: str, context: str, bargain_count: int=0) -> str:
        ...
        response = self._chat_completion(messages, temperature=dynamic_temp)
