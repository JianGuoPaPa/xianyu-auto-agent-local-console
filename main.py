...
class XianyuLive:
    ...
    def build_item_reply_profile(self, profile):
        """构建商品专属回复策略，优先级高于全局提示词。"""
        if not profile:
            return ""

        parts = []
        delivery_text = (profile.get("delivery_text") or "").strip()
        custom_prompt = (profile.get("custom_prompt") or "").strip()
        if delivery_text:
            parts.append("【该商品自动发货状态】\n买家付款后，系统会自动发送已配置的交付内容；未付款咨询时不要提前透露具体网盘链接、卡密或下载地址。")
        if custom_prompt:
            parts.append(f"【该商品专属回复规则】\n{custom_prompt}")
        if not parts:
            return ""
        return "\n\n".join(parts)

    async def handle_auto_delivery_event(self, message, websocket):
        """付款待发货事件触发后，按当前商品配置发送真实交付内容。"""
        event = extract_pending_delivery_event(message)
        if not event.get("triggered"):
            return False

        chat_id = event.get("chat_id")
        target = self.context_manager.get_latest_chat_delivery_target(chat_id) if chat_id else {}
        item_id = event.get("item_id") or target.get("item_id")
        buyer_id = event.get("buyer_id") or target.get("user_id")
        if buyer_id == chat_id and target.get("user_id"):
            buyer_id = target.get("user_id")

        if not chat_id or not item_id or not buyer_id:
            logger.warning(
                f"自动发货触发但缺少必要信息，跳过: chat_id={chat_id}, item_id={item_id}, buyer_id={buyer_id}, reason={event.get('reason')}"
            )
            return False

        profile = self.context_manager.get_item_reply_profile(item_id)
        delivery_text = (profile or {}).get("delivery_text", "").strip()
        if not delivery_text:
            logger.warning(f"商品 {item_id} 未配置付款后自动发货内容，跳过自动发货")
            return False

        delivery_key = build_delivery_key(event.get("order_id"), chat_id, item_id, buyer_id)
        reserved = self.context_manager.reserve_auto_delivery(
            delivery_key,
        ...
        if not reserved:
            logger.info(f"自动发货记录已存在，跳过重复发送: {delivery_key}")
            return True

        try:
            await self.send_msg(websocket, chat_id, buyer_id, delivery_text)
            self.context_manager.add_message_by_chat(chat_id, self.myid, item_id, "assistant", delivery_text)
            self.context_manager.mark_auto_delivery_result(delivery_key, "success")
            logger.info(f"自动发货完成: delivery_key={delivery_key}, item_id={item_id}, buyer_id={buyer_id}")
            return True
        except Exception as e:
            error = str(e)
            self.context_manager.mark_auto_delivery_result(delivery_key, "failed", error)
            logger.error(f"自动发货失败: delivery_key={delivery_key}, error={error}")
            return False
        ...
        try:
            ...
            try:
                ...
                if message['3']['redReminder'] == '等待买家付款':
                    ...
                    await self.handle_auto_delivery_event(message, websocket)
                    return
            ...
            if self.is_bracket_system_message(send_message):
                if await self.handle_auto_delivery_event(message, websocket):
                    return
            ...
            item_reply_profile = self.context_manager.get_item_reply_profile(item_id)
            item_reply_profile_text = self.build_item_reply_profile(item_reply_profile)
            if item_reply_profile_text:
                item_description += (
                    "\n\n【重要】下面是当前商品的专属回复策略，优先级高于通用提示词。"
                    f"\n{item_reply_profile_text}"
                )
...
if __name__ == '__main__':
    ...
    if os.name == "nt" and os.path.exists(".env.windows.example"):
        load_dotenv(".env.windows.example", encoding="utf-8-sig")
        logger.info("已加载 Windows 默认配置")
    
    if os.path.exists(".env.example"):
        load_dotenv(".env.example", encoding="utf-8-sig")  # 不会覆盖已存在的变量
